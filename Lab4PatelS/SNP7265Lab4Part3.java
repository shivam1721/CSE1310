...
public class SNP7265Lab4Part3 {
    ...
    final int MAXROBOTS = 30;
        ...
        ArrayList<Double> robotDecCost = new ArrayList<>();
        ArrayList<Character> robotDecQuant = new ArrayList<>();
        ...
        String fileName = "SNP7265Lab4Part3";
        ...
        catch (FileNotFoundException fnfe)
        {
            ...
            System.out.println("Input File SNP7265Lab4Part3.txt not found or output file not found ");  
        ...
        while (inputFile.hasNext())
        {
            ...
            if(inputValid)
            {
                ...
                if (robotIvalues[ri][0] != 0)
                {
                    ...
                    robotDecCost.add(Double.parseDouble(tempRCost));
                ...
                robotDecQuant.add(tempCostValue.charAt(0));
    ...
    bubbleSort(robotSvalues,robotIvalues,robotDvalue,robotCvalues,outFile,sortBy,robotDecCost,robotDecQuant);
    ...
    for (ri = 0; ri< robotIvalues.length; ri++)
        ...
        System.out.printf("\n%-36sTable of Robot Info with Height, Weight, Implementation and Actor","");
        outFile.printf("\n%-36sTable of Robot Info with Height, Weight, Implementation and Actor","");
        System.out.println("");
        outFile.println("");
       
        
        System.out.printf("%16s%6s%10s%13s%20s%40s%13s%11s   %6s%18s   %-40s\n","NAME","YEAR","COST","SOURCE","CATEGORY","TITLE","HEIGHT","WEIGHT","IMPLEMENTED","ACTOR","DOUBLE");
        outFile.printf("%16s%6s%10s%13s%20s%40s%13s%11s   %6s%18s   %-40s\n","NAME","YEAR","COST","SOURCE","CATEGORY","TITLE","HEIGHT","WEIGHT","IMPLEMENTED","ACTOR","DOUBLE");
        
                
        
    robotNTable(robotSvalues,robotIvalues,robotCvalues,outFile,robotDvalue);
    
        System.out.println(" ");
        
        
     
        System.out.printf("\n%-36sTable of Robot Info, Sorted by Name","");
        outFile.printf("\n%-36sTable of Robot Info, Sorted by Name","");
        System.out.println("");
        outFile.println("");
        ...
        bubbleASort(robotSvalues,robotIvalues,robotDvalue,robotCvalues,outFile,sortBy,robotDecCost,robotDecQuant);
    for (int i = 0; i< robotIvalues.length; i++)
        if(robotIvalues[i][0] != 0)
        {
             System.out.printf("%16s%6d%10s%10c%23s%40s%12.1f%12.1f     %4c%23s   %-40.1f\n",
                        robotSvalues[i][0],robotIvalues[i][0],robotSvalues[i][3],robotCvalues[i][0],robotSvalues[i][1],robotSvalues[i][2],
                        robotDvalue[i][1],robotDvalue[i][2],robotCvalues[i][1],robotSvalues[i][4],robotDvalue[i][3]);
            outFile.printf("%16s%6d%10s%10c%23s%40s%12.1f%12.1f     %4c%23s   %-40.1f\n",
                        robotSvalues[i][0],robotIvalues[i][0],robotSvalues[i][3],robotCvalues[i][0],robotSvalues[i][1],robotSvalues[i][2],
                        robotDvalue[i][1],robotDvalue[i][2],robotCvalues[i][1],robotSvalues[i][4],robotDvalue[i][3]);
        }
    
        System.out.printf("\n%-36sTable of Robot Info with ArrayList","");
        outFile.printf("\n%-36sTable of Robot Info with ArrayList","");
        System.out.println("");
        outFile.println("");
       
        System.out.printf("%16s%6s%13s%12s%20s%40s%13s%11s   %6s%18s   %-40s\n","NAME","YEAR","COST","SOURCE","CATEGORY","TITLE","HEIGHT","WEIGHT","IMPLEMENTED","ACTOR","DOUBLE");
        outFile.printf("%16s%6s%13s%12s%20s%40s%13s%11s   %6s%18s   %-40s\n","NAME","YEAR","COST","SOURCE","CATEGORY","TITLE","HEIGHT","WEIGHT","IMPLEMENTED","ACTOR","DOUBLE");
        
        
        bubbleArraySort(robotSvalues,robotIvalues,robotDvalue,robotCvalues,outFile,robotDecCost,robotDecQuant,sortBy);
    for (int i = 0; i< robotDecCost.size(); i++)
        if(robotIvalues[i][0] != 0)
        {
             System.out.printf("%16s%6d%10.1f%4c%8c%23s%40s%12.1f%12.1f     %4c%23s   %-40.1f\n",
                        robotSvalues[i][0],robotIvalues[i][0],robotDecCost.get(i),robotDecQuant.get(i),robotCvalues[i][0],robotSvalues[i][1],robotSvalues[i][2],
                        robotDvalue[i][1],robotDvalue[i][2],robotCvalues[i][1],robotSvalues[i][4],robotDvalue[i][3]);
               
             outFile.printf("%16s%6d%10.1f%4c%8c%23s%40s%12.1f%12.1f     %4c%23s   %-40.1f\n",
                        robotSvalues[i][0],robotIvalues[i][0],robotDecCost.get(i),robotDecQuant.get(i),robotCvalues[i][0],robotSvalues[i][1],robotSvalues[i][2],
                        robotDvalue[i][1],robotDvalue[i][2],robotCvalues[i][1],robotSvalues[i][4],robotDvalue[i][3]);
    ...
    public static void bubbleSort(String[][] robotSvalues , int[][] robotIvalues, double[][] robotDvalue, char[][] robotCvalues, PrintWriter outFile, int sortBy,ArrayList<Double> robotDecCost, ArrayList<Character> robotDecQuant)
    {
         int[] tmp; 
            ...
            {    
              if((robotDvalue[j][sortBy]!=0) && (robotDvalue[j+1][sortBy]!=0))
                if ((robotDvalue[j][sortBy] > robotDvalue[j+1][sortBy]))
                {
                    ...
                    arrlist = robotDecCost.get(j+1);
                    arrli = robotDecCost.get(j);
                    robotDecCost.set(j, arrlist);
                    robotDecCost.set(j+1, arrli);
                    
                    aa = robotDecQuant.get(j+1);
                    ab = robotDecQuant.get(j);
                    robotDecQuant.set(j, aa);
                    robotDecQuant.set(j+1, ab);
    ...
    public static void robotNTable(String[][] robotSvalues , int[][] robotIvalues, char[][] robotCvalues, PrintWriter outFile, double[][] robotDvalue )
            
    {
        for (int ri = 0; ri < robotIvalues.length; ri++)
            if (robotIvalues[ri][0] != 0)
            {
                ...
                System.out.printf("%16s%6d%10s%10c%23s%40s%12.1f%12.1f     %4c%23s   %-40.1f\n",
                        robotSvalues[ri][0],robotIvalues[ri][0],robotSvalues[ri][3],robotCvalues[ri][0],robotSvalues[ri][1],robotSvalues[ri][2],
                        robotDvalue[ri][1],robotDvalue[ri][2],robotCvalues[ri][1],robotSvalues[ri][4],robotDvalue[ri][3]);
                outFile.printf("%16s%6d%10s%10c%23s%40s%12.1f%12.1f     %4c%23s   %-40.1f\n",
                        robotSvalues[ri][0],robotIvalues[ri][0],robotSvalues[ri][3],robotCvalues[ri][0],robotSvalues[ri][1],robotSvalues[ri][2],
                        robotDvalue[ri][1],robotDvalue[ri][2],robotCvalues[ri][1],robotSvalues[ri][4],robotDvalue[ri][3]);
    ...
    public static void bubbleASort(String[][] robotSvalues , int[][] robotIvalues, double[][] robotDvalue, char[][] robotCvalues, PrintWriter outFile, int sortBy,ArrayList<Double> robotDecCost, ArrayList<Character> robotDecQuant)
    {
         int[] tmp; 
        ...
        for(int i = 0; i < robotIvalues.length-1; i++) 
        {
            for(int j = 0; j < robotIvalues.length-1; j++)
            {    
              if((robotSvalues[j][sortBy]!= null) && (robotSvalues[j+1][sortBy]!= null))
                if ((robotSvalues[j][sortBy].compareTo(robotSvalues[j+1][sortBy]))> 0)
                {
                    temp = robotDvalue[j+1];
                    robotDvalue[j+1] = robotDvalue[j];
                    robotDvalue[j] = temp;
    ...
    public static void bubbleArraySort(String[][] robotSvalues , int[][] robotIvalues, double[][] robotDvalue, char[][] robotCvalues, PrintWriter outFile, ArrayList<Double> robotDecCost, ArrayList<Character> robotDecQuant, int sortBy)
    {
